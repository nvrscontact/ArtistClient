...
function Main() {
    ...
    <span>Buy your tickets for <span className='text-white/80'>#Tour2025</span> before June</span>
    ...
    <>
      <header className='flex flex-col w-full relative xl:mx-96'>
          <section className='absolute z-10 top-10 flex flex-col text-center w-full left-1/2 -translate-x-1/2'>
            ...
            <p className='text-[12px] space-x-3 lg:text-sm text-white/40 mb-3'>
         ...
         <p className='absolute z-10 flex flex-col items-end bottom-10 right-6 text-end lg:bottom-8 text-white/50'>
          ...
          <button className='mt-4 flex items-center gap-2 text-sm bg-white/10 rounded-md py-3 pl-4 pr-3 w-fit text-white'>
      ...
      <aside className="lg:mx-30 xl:mx-96 mb-30 ">
        <section className='relative overflow-hidden w-full'>
          ...
          {Array(4).fill(mensajes[0]).map((mensaje,index) => (
           <p key={index} className="shrink-0 space-x-14 py-3 px-4 text-[14px] text-white/60 inter-200 flex lg:text-sm ">
            {mensaje}
          </p>
          ))}
          </div>
